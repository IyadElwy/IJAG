...
def get_current_game_state(game_state_file_path: str) -> GameState:
...
def get_game_level_data(game_data_file_path: str, level: str) -> Level:
...
def add_game_state_ordered_convo_node(
    game_state_file_path: str, level: str, convo_node_name: str
):
    with open(game_state_file_path, "r") as state_file:
        state_file_as_json = json.loads(state_file.read())
